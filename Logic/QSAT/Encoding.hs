...
class Encoded t where
    type Decoded t :: *
    type Brand t :: *
    decode :: (Literal (Brand t) -> Bool) -> t -> Decoded t
...
instance (Brand a ~ Brand b, Encoded a, Encoded b) => Encoded (a,b) where
    type Decoded (a,b) = (Decoded a, Decoded b)
    decode f (a,b) = (decode f a, decode f b)

instance (Brand a ~ Brand b, Brand a ~ Brand c, Encoded a, Encoded b, Encoded c) => Encoded (a, b, c) where
    type Decoded (a,b,c) = (Decoded a, Decoded b, Decoded c)
    decode f (a,b,c) = (decode f a, decode f b, decode f c)

instance (Brand a ~ Brand b, Brand a ~ Brand c, Brand a ~ Brand d, Encoded a, Encoded b, Encoded c, Encoded d) => Encoded (a, b, c, d) where
    type Decoded (a,b,c,d) = (Decoded a, Decoded b, Decoded c, Decoded d) 
    decode f (a,b,c,d) = (decode f a, decode f b, decode f c, decode f d)

instance (Brand a ~ Brand b, Brand a ~ Brand c, Brand a ~ Brand d, Brand a ~ Brand e, Encoded a, Encoded b, Encoded c, Encoded d, Encoded e) => Encoded (a, b, c, d, e) where
